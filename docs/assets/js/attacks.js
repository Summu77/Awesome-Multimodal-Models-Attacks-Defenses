...
attackSections.addEventListener("input", (event) => {
  handleFilterEvent(event);
});

attackSections.addEventListener("change", (event) => {
  handleFilterEvent(event);
});
...
function renderPage() {
  attackSections.dataset.viewMode = viewMode;
  const filteredGroups = attackGroups.map((group) => filterGroup(group));

  renderCategoryNav(filteredGroups);
  attackSections.innerHTML = filteredGroups.map((group) => renderGroupSection(group)).join("");
